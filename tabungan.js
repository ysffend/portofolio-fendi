...
// State sementara untuk melacak aksi hapus yang sedang aktif
let deleteActionType = null; // 'goal' atau 'logs'
let pendingDeleteGoalId = null;

// Fungsi Buka Modal Hapus Target
function openDeleteGoalModal(goalId) {
  const goal = goals.find((g) => g.id === goalId);
  if (!goal) return;

  deleteActionType = "goal";
  pendingDeleteGoalId = goalId;
  deleteModalInfo.innerHTML = `Apakah Anda yakin ingin menghapus orbit target <strong>"${goal.name}"</strong>? Sisa saldo <strong>${formatRupiah(goal.savedAmount)}</strong> di dalamnya akan hilang.`;
...
// Fungsi Buka Modal Hapus Semua Log
function openClearLogsModal() {
  if (logs.length === 0) return;

  deleteActionType = "logs";
  pendingDeleteGoalId = null;
  deleteModalInfo.innerHTML = `Apakah Anda yakin ingin menghapus <strong>seluruh log aktivitas</strong> di orbit?`;
...
confirmDeleteBtn.addEventListener("click", () => {
  if (deleteActionType === "goal") {
    // Aksi Hapus Target Tabungan
    const goal = goals.find((g) => g.id === pendingDeleteGoalId);
    if (goal) {
      goals = goals.filter((g) => g.id !== pendingDeleteGoalId);
      addLog(`Orbit "${goal.name}" dihapus`, "system", 0);
    }
  } else if (deleteActionType === "logs") {
    // Aksi Hapus Semua Log Transaksi
    logs = [];
  }
...
// Ganti handler lama ke modal custom
function handleDelete(goalId) {
